...
async function advanceAndApplyEligibleUpdatesDirect(
  account: { address: `0x${string}` } | `0x${string}`,
  publicClient: any,
  walletClient: any,
  testClient: any,
...
async function findSavedUsedPoolUpwardDualArchetypeMatches(
  ...
  for (const archetype of EXPERIMENTAL_USED_POOL_UPWARD_ARCHETYPES) {
    ...
    const dualCandidate = snapshot.candidates.find((candidate) => candidate.intent === "LEND_AND_BORROW");
    observations.push(
...
describeIf("Base factory fork integration", () => {
  ...
  itBaseExperimental(
    ...
    async () => {
      ...
      for (const match of inspectedMatches) {
        ...
        expect(match.currentRateBps, observations.join(" | ")).toBeLessThan(
          match.targetRateBps
        );
      }
    },
  ...
  itBaseExperimental(
    "finds manual remove-quote-plus-draw-debt improvements on some hand-picked managed archetypes, but not on PRIME, once the dual branch must beat the matching remove-only path",
    async () => {
      const { probedMatches, observations } =
      ...
      expect(primeMatch?.improved, observations.join(" | ")).toBe(false);
      expect(
        probedMatches.some((match) => match.improved),
        observations.join(" | ")
      ).toBe(true);
