...
- Pre-window lend remains withheld as a generic live surfaced path. Experimental DWATP-threshold probes now show that pre-window `LEND` can improve a deterministic not-due step-up fixture over a two-update horizon, and there is now a targeted exact surfaced-plan proof for that pinned deterministic state. The generic exact path is still exploratory, and the heuristic path is intentionally suppressed to avoid overclaiming what the next update can do.
