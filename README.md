...
- Pre-window lend is still experimental. The exact path now has deterministic surfaced-and-executed proofs over a two-update horizon, including a threshold-aware generic proof that stays well below the configured max exposure in the saved deterministic fixture, but it is not yet broadly validated across live ongoing-pool fixtures. The heuristic path is still intentionally suppressed to avoid overclaiming what the next update can do.
