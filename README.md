...
- managed used-pool upward control through curator-style inventory-backed `REMOVE_QUOTE` is end-to-end verified on the dog/USDC archetype: the candidate surfaces, the planned call executes on the forked pool, and the realized on-chain rate after the next Ajna rate update matches the simulation prediction exactly. Across the five pinned managed archetypes, only dog/USDC surfaces a candidate today. The diagnostic probe across all five (in `tests/integration/base/curator.ts`) shows that three non-surfacing archetypes fail because of pool state (not EMA-initialized, or pinned inside the rate-update window) and one (PRIME/USDC) fails because the operator owns only ~9% of the target bucket. An independent synthetic-pool ownership sweep puts the threshold at 80%–95% operator bucket ownership on that specific synthetic construction; the threshold is pool-state-dependent, not a universal constant. Curator mode therefore requires an EMA-initialized pool, pre-window timing, AND enough operator ownership of the target bucket that the keeper's characteristic ~2/3 release fraction produces a beat-passive rate move. Operators should verify their pool surfaces a candidate in dry-run before going live. See the [Operator Runbook](./docs/curator-mode.md#operator-runbook).
