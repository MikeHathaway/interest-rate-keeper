...
The current Base-fork evidence is important: the keeper now finds a lend plan in representative borrowed-pool step-up fixtures, borrower-side lookahead planning can find and live-execute a representative multi-cycle `BORROW` plan when a wider search space is enabled, and multi-bucket quote search now surfaces, naturally selects, and live-executes a representative exact `LEND_AND_BORROW` candidate on the fork. Due-window mutating plans no longer append a separate `UPDATE_INTEREST` step, because the first mutating action can consume the overdue update itself. The fork suite now also proves that the keeper aborts safely when another actor changes the pool between planning and execution. On the borrower side, exact same-cycle `DRAW_DEBT` search still does not find a safe candidate in the representative brand-new quote-only fixture covered by routine integration, while heuristic `DRAW_DEBT` synthesis is now available as an exploratory fallback. The broader existing-borrower exact same-cycle negative proof remains in the `experimental` profile, now filtered to true borrower/loan states, and it is still negative even on a deliberately borrower-heavy existing-loan fixture set. Pre-window lend is no longer best understood as “flip the next update.” Experimental DWATP-threshold probes now show a positive manual pre-window `LEND` effect over a two-update horizon in the deterministic not-due step-up fixture, which matches Ajna’s lagged meaningful-deposit EMA mechanics. That manual-positive state now has two exact surfaced-plan proofs: one when the known-good bucket and amount are both pinned, and one when the known-good bucket is pinned but the exact engine chooses the amount itself. The live engine still withholds generic surfaced pre-window `LEND` plans across the full derived threshold-aware bucket set, because that unpinned path is still exploratory and materially more expensive than the pinned proof. Borrower-side exact mode is now useful for representative multi-cycle live steering, but not yet a proven live same-cycle borrow executor.
...
- it also proves that the deterministic not-due step-up fixture does not surface either exact or heuristic pre-window `LEND` plans in the live snapshot path
- it also includes an experimental negative proof that representative multi-actor ongoing-pool fixtures still do not surface pre-window `LEND` plans in the live snapshot path
...
- Pre-window lend remains withheld as a generic live surfaced path. Experimental DWATP-threshold probes now show that pre-window `LEND` can improve a deterministic not-due step-up fixture over a two-update horizon, and there are now targeted exact surfaced-plan proofs for that deterministic state with the known-good bucket pinned. The generic exact path across the full threshold-aware bucket set is still exploratory, and the heuristic path is intentionally suppressed to avoid overclaiming what the next update can do.
