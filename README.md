...
- it also includes experimental DWATP-threshold probes that show a positive manual pre-window `LEND` effect over a two-update horizon in the deterministic not-due fixture, even though the live exact/heuristic surfaced path is still withheld
...
- Pre-window lend remains withheld in the live engine. Experimental DWATP-threshold probes now show that pre-window `LEND` can improve a deterministic not-due step-up fixture over a two-update horizon, but the surfaced exact path is still exploratory and the heuristic path is intentionally suppressed to avoid overclaiming what the next update can do.
