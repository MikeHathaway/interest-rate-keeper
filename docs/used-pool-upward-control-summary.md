...
- targeted managed `REMOVE_QUOTE` is **supported but narrow**: end-to-end verified on exactly one pinned archetype (dog/USDC, where the operator owns ~100% of the target bucket). An expanded 8-archetype sweep (sourced via `scripts/discover-pinned-used-pools.mjs` querying the ajna.info.finance index) found no additional surfacing archetypes — most fail the eligibility gate (not EMA-initialized), and the one that passes eligibility but is new (pool `0x7b0b85c8`) hits a third dual failure mode: passive trajectory already saturates the protocol's 2× STEP_UP ceiling over the 2-update lookahead, so no action can improve on it. Two distinct fractions matter and should not be confused: the operator's **ownership fraction** of the bucket (gates whether a candidate surfaces at all) and the candidate's **release fraction** (how much of the bucket the surfaced action withdraws). The diagnostic probe across all five pinned managed archetypes in `tests/integration/base/curator.ts` shows three archetypes failing for correct keeper-side reasons (not EMA-initialized, or pinned inside the rate-update window) and one (PRIME/USDC, ~9% operator ownership) failing because the ownership is too low for the rate response to beat passive. A synthetic-pool two-shape rate-response comparison further shows that the STEP_UP cliff position varies materially with pool state — the baseline synthetic pool cliffs at ~40% withdrawal, a 5×-higher-debt synthetic shape has no viable cliff at any fraction (all withdrawals STEP_DOWN), and dog/USDC cliffs at ~66.5%. There is no universal 2/3 rule; the cliff is a function of debt/deposit ratio + EMA state + target offset. Curator mode therefore requires (a) an EMA-initialized pool pinned pre-window and (b) a pool state where the operator's withdrawal can actually move MAU/TU across the STEP_UP threshold. Dry-run is the definitive per-pool test. See [Curator Mode](./curator-mode.md) for the per-archetype breakdown, ownership sweep, and rate-response cliff comparison.
- managed `REMOVE_QUOTE + DRAW_DEBT` (dual) remains **research**. Expanded empirical characterization across 10 total archetypes (8 Base + 2 Ethereum mainnet, with a gate relaxation that now permits solo-borrower managed pools to enter dual synthesis) found zero dual candidates surfacing. Three distinct failure modes are now characterized. On real-chain PRIME/USDC, a debug-trace diagnostic shows 94% of the search's 288 evaluated tuples revert during `drawDebt` simulation because the borrower lacks collateral headroom — a pool-state constraint, not a search bug. On a synthetic dual-viable fixture (95% operator ownership + same-account borrower + 20× collateral headroom), the code evaluates tuples cleanly (~50% success rate), but single-action REMOVE_QUOTE alone already saturates the protocol's 2× STEP_UP ceiling over the 2-update lookahead, so dual cannot strictly improve on single — a protocol-level ceiling interacting with the "must beat single" comparison rule. Dual will ship-able only when a pool state exists where single-action is marginal enough to leave STEP_UP headroom AND the borrower has enough collateral to draw meaningfully AND the operator has concentrated bucket ownership. See [Curator Mode](./curator-mode.md) for the full diagnostic.
